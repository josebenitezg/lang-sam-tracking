...
from lang_sam.lang_dino import predict_dino
...
def get_prompt(click_state, click_input):
    ...
    print('prompt state: ', click_state)
    prompt = {
        "prompt_type":["click"],
...
def init_SegTracker_Text(aot_model, sam_gap, max_obj_num, points_per_side, origin_frame, text_prompt):
    ...
    if text_prompt is not None:
        boxes, logits, phrases = predict_dino(origin_frame, text_prompt, box_threshold=0.3, text_threshold=0.25)
        if boxes is not None:
            frame_with_bbox = draw_bbox(origin_frame, boxes)
        print(boxes, logits, phrases)
    else:
        frame_with_bbox = origin_frame    
        
    
    return Seg_Tracker, origin_frame, [[], []], frame_with_bbox
...
# draw bounding box consideringa having this format  boxes = box_ops.box_cxcywh_to_xyxy(boxes) * torch.Tensor([W, H, W, H])
...
def bbox_from_dino(Seg_Tracker, origin_frame, text_prompt, aot_model, sam_gap, max_obj_num, points_per_side):
    """
    Args:
        template_frame: PIL.Image
        point_prompt: flag for positive or negative button click
        click_state: [[points], [labels]]
    """
    print('processing bbox from dino')
    if Seg_Tracker is None:
        Seg_Tracker, _, _ = init_SegTracker(aot_model, sam_gap, max_obj_num, points_per_side, origin_frame)

    # prompt for dino MODEL
    boxes, logits, phrases = predict_dino(origin_frame, text_prompt, box_threshold=0.3, text_threshold=0.25)
    ...
    boxes = boxes.numpy().reshape(2, 2).round().astype(int)
    predicted_mask, masked_frame = Seg_Tracker.seg_acc_bbox(origin_frame, boxes)
...
def seg_track_app():
        ...
        with gr.Row():
            ...
            with gr.Column(scale=0.5):
                ...
                with tab_text:
                    ...
                    with gr.Row():
                        submit_text_prompt = gr.Button(value='Submit', interactive=True)
                        reset_text_prompt = gr.Button(value='Reset', interactive=True)
    ...
    ##########################################################
        ...
        # Interactively draw bbox acc text
        submit_text_prompt.click(
            fn=bbox_from_dino,
            inputs=[
                Seg_Tracker, origin_frame, text_prompt,
                aot_model,
                sam_gap,
                max_obj_num,
                points_per_side,
